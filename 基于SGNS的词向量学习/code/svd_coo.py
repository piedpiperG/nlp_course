...
class Svd_dec:
    ...
    def build_cooccurrence_matrix(self, sentences):
        ...
        print("共现矩阵构建完成。")
        return cooccurrence_matrix, vocab_index  # 将结果转换为 CSR 格式
    ...
    def apply_svd(self, cooccurrence_matrix):
        ...
        # 计算非零奇异值的总数
        total_non_zero_singular_values = np.sum(Sigma > 0)

        # 选取的奇异值的数量
        selected_singular_values = k

        # 选取的奇异值之和
        selected_singular_values_sum = np.sum(Sigma[:k])

        # 全部奇异值之和
        total_singular_values_sum = np.sum(Sigma)

        # 二者的比例
        ratio = selected_singular_values_sum / total_singular_values_sum

        print(f"总共有 {total_non_zero_singular_values} 个非零奇异值")
        print(f"选取了 {selected_singular_values} 个奇异值")
